...
bool Player::getAnimeExist(const std::string& animeName) {
	AnimeRef* animeRef = _currentRs->animeCache->getReference(animeName);
	if (animeRef == NULL)return false;
	return true;
...
//					sprite->_state;												//SpriteStudio上のアトリビュートの値は_stateから取得してください
					...
					result.rotationZ = sprite->_state.Calc_rotationZ;				// Z回転（親子関係計算済）
					...
					result.scaleX = sprite->_state.Calc_scaleX;
					result.scaleY = sprite->_state.Calc_scaleY;
...
Quad Player::getPartQuad(const char* name) {
		...
		else
		{
			...
			else
			{
				if (sprite->_state.texture.handle != -1)
				{
					if ((sprite->_state.isVisibled == true) && (sprite->_state.opacity > 0))
					{
						SSDrawSpriteShadow(sprite->_state);
						_draw_count++;
					}
				}
			}
